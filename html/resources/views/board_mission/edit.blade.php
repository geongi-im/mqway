...
<div class="relative bg-[#3D4148] py-12 overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        <h1 class="text-3xl font-bold text-white mb-2">미션 수정</h1>
        <p class="text-gray-400 text-sm">작성한 미션의 내용을 수정합니다.</p>
    </div>
</div>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-8 md:p-10">
                <form action="{{ route('board-mission.update', $post->idx) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select name="mq_category" 
                                    id="mq_category"
                                    class="w-full h-12 pl-4 pr-10 border border-gray-200 rounded-xl focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FF6B35]/20 transition-all bg-gray-50 cursor-pointer appearance-none font-medium text-gray-700 @error('mq_category') border-red-500 @enderror"
                                    required>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" 
                               ...
                               class="w-full h-12 px-4 border border-gray-200 rounded-xl focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FF6B35]/20 transition-all bg-gray-50 placeholder-gray-400 font-medium @error('mq_title') border-red-500 @enderror"
                    ...
                    <div class="space-y-6 pt-6 border-t border-gray-100">
                                    ...
                                    @foreach($post->mq_image as $index => $filename)
                                        <div class="relative group" id="existing-image-{{ $index }}">
                                            <div class="aspect-square rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                                                <img src="{{ asset('storage/uploads/board_mission/' . $filename) }}" 
                                                     alt="첨부 이미지" 
                        ...
                        <div>
                            <div class="flex justify-between items-end mb-3">
                                ...
                                <button type="button" 
                                        class="text-sm px-3 py-1.5 bg-[#FF6B35]/10 text-[#D4561F] hover:bg-[#FF6B35]/20 rounded-lg transition-colors font-medium flex items-center gap-1" 
                            ...
                            <div id="fileUploadContainer" class="space-y-3">
                                ...
                                <div class="file-input-group animate-fadeIn" data-index="0">
                                    <div class="relative group">
                                        ...
                                        <div class="w-full h-12 px-4 border border-gray-200 rounded-xl bg-gray-50 flex items-center justify-between cursor-pointer group-hover:border-[#FF6B35] transition-all">
                                            ...
                                            <svg class="w-5 h-5 text-gray-400 group-hover:text-[#FF6B35] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                        </div>
                                    </div>
                    ...
                    <div class="flex items-center justify-between pt-8 border-t border-gray-100 mt-8">
                        <button type="button" 
                                onclick="if(confirm('수정 중인 내용이 저장되지 않고 사라집니다.\n이전 페이지로 돌아가시겠습니까?')) { location.href='{{ route('board-mission.show', $post->idx) }}'; }"
...
<script>
    class UploadAdapter {
        ...
        _initRequest() {
            ...
            xhr.open('POST', '{{ route('board-mission.upload.image') }}', true);
    ...
    function deleteImage(button, filename) {
        fetch(`/board-mission/delete-image/{{ $post->idx }}/${filename}`, {
            method: 'POST',
            headers: {
    ...
    function previewAttachment(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if(iconContainer) iconContainer.innerHTML = '<svg class="w-4 h-4 text-[#FF6B35]" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>';
            };
