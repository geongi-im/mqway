...
<section class="relative pt-32 pb-24 overflow-hidden bg-[#3D4148]">
    <div class="absolute inset-0">
        ...
        <div class="absolute -top-24 -right-24 w-96 h-96 bg-[#FF6B35] rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob"></div>
    ...
    <div class="container mx-auto px-4 relative z-10 text-center animate-slideUp">
        ...
        <p class="text-xl text-gray-300 max-w-2xl mx-auto leading-relaxed font-light">
            주어진 미션을 기록하고 달성해보세요.<br class="hidden md:block">
            작은 실천이 큰 변화를 만듭니다.
        </p>

        @if(auth()->user()->mq_level < 10)
        <div class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-gray-400 backdrop-blur-md">
...
<div class="container mx-auto px-4 -mt-10 relative z-20 mb-10 animate-slideUp" style="animation-delay: 0.2s;">
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8 max-w-4xl mx-auto">
        <form action="{{ route('board-mission.index') }}" method="GET">
            <!-- 현재 필터 상태 유지 -->
            @if(request('category'))
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative flex-grow">
                    <input type="text" 
                           ...
                           class="w-full h-12 pl-4 pr-12 bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-2 focus:ring-[#FF6B35] focus:border-transparent transition-all placeholder-gray-400" 
                           placeholder="미션 제목이나 내용을 검색해보세요">
                    <button type="submit" class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-[#FF6B35] transition-colors" title="검색">
                ...
                <a href="{{ route('board-mission.index') }}" class="h-10 w-10 flex-shrink-0 flex items-center justify-center text-gray-400 hover:text-[#FF4D4D] hover:bg-red-50 rounded-lg transition-all" title="초기화">
                ...
                <!-- 글쓰기 -->
                @auth
                <a href="{{ route('board-mission.create') }}" 
...
<div class="container mx-auto px-4 mb-8 max-w-7xl animate-slideUp" style="animation-delay: 0.3s;">
    <form action="{{ route('board-mission.index') }}" method="GET" id="filterForm">
        <!-- 검색어 상태 유지 -->
        @if(request('search'))
        ...
        <div class="flex flex-row items-center justify-between gap-4">
            ...
            <div class="relative">
                <select name="category" 
                        ...
                        class="appearance-none h-9 pl-3 pr-9 bg-white border border-gray-200 text-gray-600 text-sm rounded-full focus:ring-2 focus:ring-[#FF6B35]/30 focus:border-[#FF6B35] hover:border-gray-300 transition-all cursor-pointer font-medium">
...
<div class="container mx-auto px-4 pb-20 max-w-7xl animate-slideUp" style="animation-delay: 0.4s;">
    @if($posts->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 px-4 bg-white rounded-3xl shadow-sm border border-gray-100 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-2">아직 등록된 미션이 없습니다</h3>
            <p class="text-gray-500 max-w-md mx-auto">
                @if(request('search'))
                    '{{ request('search') }}'에 대한 검색 결과가 없습니다.<br>
                    다른 키워드로 검색해보시거나 필터를 변경해보세요.
                @else
                    첫 번째 미션을 등록하고 목표를 향해 나아가보세요!<br>
                    작은 시작이 큰 변화를 만듭니다.
                @endif
            </p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                @foreach($posts as $post)
                    <a href="{{ route('board-mission.show', $post->idx) }}" 
                       class="block group hover:bg-[#FFF8F5] transition-colors duration-200">
                        ...
                        <div class="hidden md:grid md:grid-cols-12 gap-4 px-6 py-4 items-center">
                            ...
                            <div class="col-span-5">
                                <h3 class="text-sm font-semibold text-gray-800 group-hover:text-[#FF6B35] transition-colors truncate">
                                    {{ $post->mq_title }}
                        ...
                        <div class="md:hidden px-5 py-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <h3 class="text-sm font-semibold text-gray-800 group-hover:text-[#FF6B35] transition-colors line-clamp-1 mb-1.5">
                                        {{ $post->mq_title }}
                                        @if(!empty($post->mq_image) && is_array($post->mq_image) && count($post->mq_image) > 0)
