...
<div class="relative bg-[#3D4148] pb-32 overflow-hidden">
    ...
    <div class="container mx-auto px-4 pt-32 relative z-10">
        <div class="max-w-4xl mx-auto">
            <a href="{{ route('board-mission.index') }}" class="inline-flex items-center text-gray-400 hover:text-white mb-6 transition-colors group">
            ...
            <div class="flex items-center justify-between border-t border-white/10 pt-6 animate-slideUp" style="animation-delay: 0.2s;">
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF6B35] to-[#E85D26] flex items-center justify-center text-white font-bold shadow-lg">
                            {{ mb_substr($post->mq_user_id, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-white font-medium">{{ $post->mq_user_id }}</p>
                            <p class="text-xs text-gray-400">Mission Writer</p>
                        </div>
                    </div>
                </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden animate-slideUp" style="animation-delay: 0.3s;">
            <div class="p-8 md:p-12">
                ...
                <!-- 본문 내용 -->
                <div class="prose prose-lg max-w-none prose-headings:text-[#2D3047] prose-p:text-gray-600 prose-a:text-[#FF6B35] prose-strong:text-[#2D3047] prose-img:rounded-xl prose-img:shadow-lg">
                    {!! $post->mq_content !!}
                </div>

                <!-- Bottom Like CTA -->
                <div class="flex flex-col items-center justify-center py-12 mt-12 mb-8 border-y border-gray-100 bg-gray-50/50 rounded-2xl">
                    <p class="text-[#2D3047] font-bold text-lg mb-6">이 미션이 도움이 되셨나요?</p>
                    <button onclick="likePost(event, {{ $post->idx }})"
                            id="bottomLikeBtn"
                ...
                <div class="mt-12 pt-8 border-t border-gray-100 flex justify-between items-center">
                    <a href="{{ route('board-mission.index') }}" 
                    ...
                    <div class="flex items-center gap-3">
                        @if(auth()->check() && (auth()->user()->mq_level >= 10 || auth()->user()->mq_user_id === $post->mq_user_id))
                            <a href="{{ route('board-mission.edit', $post->idx) }}" 
                            ...
                            <form action="{{ route('board-mission.destroy', $post->idx) }}"
                                  method="POST"
...
async function likePost(event, idx) {
    ...
    try {
        ...
        const response = await fetch(`/board-mission/${idx}/like`, {
            method: 'POST',
            headers: {
