...
<div class="min-h-screen bg-primary">
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-8">
            ...
            <form action="{{ route('board-insights.update', $post->idx) }}" method="POST" enctype="multipart/form-data" class="space-y-6" onsubmit="showLoadingManager()">
                @csrf
                @method('PUT')
                                ...
                                <div class="file-input-group relative mb-3">
                                    <div class="mb-2">
                                        <div class="relative inline-block">
                                            <a href="{{ asset('storage/uploads/board_insights/' . $filename) }}"
                                               target="_blank"
...
function deleteThumbnail(button) {
    fetch(`/board-insights/{{ $post->idx }}/thumbnail`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            'Accept': 'application/json',
