...
<div class="relative bg-[#3D4148] py-12 overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        <h1 class="text-3xl font-bold text-white mb-2">인사이트 수정</h1>
        <p class="text-gray-400 text-sm">작성한 인사이트의 내용을 수정합니다.</p>
    </div>
</div>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-8 md:p-10">
                <form action="{{ route('board-insights.update', $post->idx) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                                    ...
                                    @foreach($post->mq_image as $index => $filename)
                                        <div class="relative group" id="existing-image-{{ $index }}">
                                            <div class="aspect-square rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                                                <img src="{{ asset('storage/uploads/board_insights/' . $filename) }}" 
                                                     alt="첨부 이미지" 
                    ...
                    <div class="flex items-center justify-between pt-8 border-t border-gray-100 mt-8">
                        <button type="button" 
                                onclick="if(confirm('수정 중인 내용이 저장되지 않고 사라집니다.\n이전 페이지로 돌아가시겠습니까?')) { location.href='{{ route('board-insights.show', $post->idx) }}'; }"
...
<script>
    class UploadAdapter {
        ...
        _initRequest() {
            ...
            xhr.open('POST', '{{ route('board-insights.upload.image') }}', true);
    ...
    function deleteImage(button, filename) {
        fetch(`/board-insights/delete-image/{{ $post->idx }}/${filename}`, {
            method: 'POST',
            headers: {
