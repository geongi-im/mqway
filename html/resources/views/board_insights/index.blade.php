...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
    <div class="mb-8 text-center">
        <h1 class="text-3xl font-bold text-dark mb-2">투자 인사이트</h1>
    </div>
    ...
    <div class="bg-primary rounded-xl p-4 mb-6">
        <form action="{{ route('board-insights.index') }}" method="GET">
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <a href="{{ route('board-insights.index') }}" class="inline-flex items-center justify-center h-12 px-5 bg-point text-cdark rounded-xl hover:bg-opacity-90 transition-all">
    ...
    <form id="filterForm" action="{{ route('board-insights.index') }}" method="GET" class="hidden">
                    ...
                    @foreach($posts as $post)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $categoryColors[$post->mq_category] ?? 'bg-blue-100 text-blue-800' }}">
                                    {{ $post->mq_category }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <a href="{{ route('board-insights.show', $post->idx) }}" class="text-dark hover:text-yellow-500 font-medium">
                                    {{ $post->mq_title }}
                                    @if(!empty($post->mq_image) && count($post->mq_image) > 0)
                                        <svg class="w-4 h-4 inline-block text-gray-400 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
