...
<section class="relative pt-32 pb-24 overflow-hidden bg-[#3D4148]">
    <div class="absolute inset-0">
        ...
        <div class="absolute -top-24 -right-24 w-96 h-96 bg-[#FFD700] rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob"></div>
        <div class="absolute -bottom-24 -left-24 w-96 h-96 bg-[#FF8C00] rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000"></div>
    </div>
    ...
    <div class="container mx-auto px-4 relative z-10 text-center animate-slideUp">
        ...
        <p class="text-xl text-gray-300 max-w-2xl mx-auto leading-relaxed font-light">
            세계적인 투자 대가들의 포트폴리오를 분석합니다.<br class="hidden md:block">
            대가의 투자 철학과 전략을 직접 확인해보세요.
        </p>
    </div>
</section>
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative flex-grow">
                    <input type="text" 
                           ...
                           class="w-full h-12 pl-4 pr-12 bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-2 focus:ring-[#FFD700] focus:border-transparent transition-all placeholder-gray-400" 
                           placeholder="투자자 이름이나 종목을 검색해보세요">
                    <button type="submit" class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-[#FFD700] transition-colors" title="검색">
                ...
                <a href="{{ route('board-portfolio.index') }}" class="h-10 w-10 flex-shrink-0 flex items-center justify-center text-gray-400 hover:text-[#FF4D4D] hover:bg-red-50 rounded-lg transition-all" title="초기화">
                ...
                <a href="{{ route('board-portfolio.create') }}" 
                   class="h-10 w-10 flex-shrink-0 flex items-center justify-center text-gray-400 hover:text-[#FFD700] hover:bg-[#FFD700]/10 rounded-lg transition-all" title="새 글 작성">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="container mx-auto px-4 mb-8 max-w-7xl animate-slideUp" style="animation-delay: 0.3s;">
    <form action="{{ route('board-portfolio.index') }}" method="GET" id="filterForm">
        <!-- 검색어 상태 유지 -->
        @if(request('search'))
        ...
        <div class="flex flex-row items-center justify-between gap-4">
            ...
            <div class="relative">
                <select name="category" 
                        ...
                        class="appearance-none h-9 pl-3 pr-9 bg-white border border-gray-200 text-gray-600 text-sm rounded-full focus:ring-2 focus:ring-[#FFD700]/30 focus:border-[#FFD700] hover:border-gray-300 transition-all cursor-pointer font-medium">
    ...
    @if($posts->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 px-4 bg-white rounded-3xl shadow-sm border border-gray-100 text-center">
            ...
            <p class="text-gray-500 max-w-md mx-auto">
                ...
                @else
                    곧 투자대가들의 포트폴리오가 업데이트될 예정입니다.<br>
                    잠시만 기다려주세요!
                @endif
            </p>
        </div>
            ...
            <div class="divide-y divide-gray-100">
                @foreach($posts as $post)
                    <a href="{{ route('board-portfolio.show', $post->idx) }}" 
                       class="block group hover:bg-[#FFFDF5] transition-colors duration-200">
                        ...
                        <div class="hidden md:grid md:grid-cols-12 gap-4 px-6 py-4 items-center">
                            ...
                            <div class="col-span-2">
                                <span class="inline-block px-2.5 py-1 text-xs font-bold rounded-full {{ $categoryColors[$post->mq_investor_code] ?? 'bg-blue-100 text-blue-800' }}">
                            ...
                            <div class="col-span-5">
                                <h3 class="text-sm font-semibold text-gray-800 group-hover:text-[#DAA520] transition-colors truncate">
                                    {{ $post->mq_title }}
                                </h3>
                            </div>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1.5">
                                        <span class="inline-block px-2 py-0.5 text-[10px] font-bold rounded-full {{ $categoryColors[$post->mq_investor_code] ?? 'bg-blue-100 text-blue-800' }}">
                                            {{ strtoupper($post->mq_investor_code) }}
                                        </span>
                                    </div>
                                    <h3 class="text-sm font-semibold text-gray-800 group-hover:text-[#DAA520] transition-colors line-clamp-1 mb-1.5">
                                        {{ $post->mq_title }}
                                    </h3>
