...
<section class="relative pt-32 pb-24 overflow-hidden bg-[#3D4148]">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center animate-slideUp">
        ...
        <p class="text-xl text-gray-300 max-w-2xl mx-auto leading-relaxed font-light">
            성공적인 투자를 위한 깊이 있는 분석과 전망을 제공합니다.<br class="hidden md:block">
            전문적인 리서치 자료를 통해 투자의 방향을 잡아보세요.
        </p>
    </div>
</section>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8 max-w-4xl mx-auto">
        <form action="{{ route('board-research.index') }}" method="GET">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('board-research.index') }}" class="h-10 w-10 flex-shrink-0 flex items-center justify-center text-gray-400 hover:text-[#FF4D4D] hover:bg-red-50 rounded-lg transition-all" title="초기화">
...
<div class="container mx-auto px-4 mb-8 max-w-7xl animate-slideUp" style="animation-delay: 0.3s;">
    <form action="{{ route('board-research.index') }}" method="GET" id="filterForm">
        <!-- 검색어 상태 유지 -->
        @if(request('search'))
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($posts as $post)
                <article class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group flex flex-col h-full">
                    <a href="{{ route('board-research.show', $post->idx) }}" class="block relative overflow-hidden aspect-video bg-gray-100">
                        @if($post->mq_image)
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <a href="{{ route('board-research.show', $post->idx) }}" class="block mb-4">
