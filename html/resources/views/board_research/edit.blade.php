...
<div class="relative bg-[#3D4148] py-12 overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        <h1 class="text-3xl font-bold text-white mb-2">리서치 수정</h1>
        <p class="text-gray-400 text-sm">작성한 리서치 게시글의 내용을 수정합니다.</p>
    </div>
</div>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-8 md:p-10">
                <form action="{{ route('board-research.update', $post->idx) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex items-center justify-between pt-8 border-t border-gray-100 mt-8">
                        <button type="button" 
                                onclick="if(confirm('수정 중인 내용이 저장되지 않고 사라집니다.\n이전 페이지로 돌아가시겠습니까?')) { location.href='{{ route('board-research.show', $post->idx) }}'; }"
...
<script>
    ...
    function deleteImage(button, filename) {
        fetch(`/board-research/delete-image/{{ $post->idx }}/${filename}`, {
            method: 'POST',
            headers: {
    ...
    function previewThumbnail(input) {
        ...
        const existingThumbnail = document.querySelector('#currentThumbnail');
        ...
        if (input.files && input.files[0]) {
            ...
            if (file.size > 2 * 1024 * 1024) {
                ...
                input.value = '';
                label.textContent = existingThumbnail ? '썸네일 이미지 변경하기' : '이미지를 선택하거나 드래그하세요';
                return;
            }
    ...
    function addFileInput() {
        ...
        // 첫 번째 요소에는 삭제 버튼이 없을 수 있으므로 항상 체크
