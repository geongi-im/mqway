...
<div class="relative bg-[#3D4148] py-12 overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center">
        <h1 class="text-3xl font-bold text-white mb-2">동영상 수정</h1>
        <p class="text-gray-400 text-sm">등록한 경제 영상을 수정합니다.</p>
    </div>
</div>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-8 md:p-10">
                <form action="{{ route('board-video.update', $post->idx) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <!-- 비디오 URL -->
                    <div class="space-y-2">
                        <label for="mq_url" class="text-sm font-semibold text-[#2D3047] block">동영상 URL <span class="text-red-500">*</span></label>
                        <input type="url" 
                               name="mq_url" 
                               id="mq_url" 
                               value="{{ old('mq_video_url', $post->mq_video_url) }}"
                               class="w-full h-12 px-4 border border-gray-200 rounded-xl focus:outline-none focus:border-[#9F5AFF] focus:ring-2 focus:ring-[#9F5AFF]/20 transition-all bg-gray-50 placeholder-gray-400 font-medium @error('mq_video_url') border-red-500 @enderror"
                               placeholder="YouTube 또는 Vimeo URL을 입력하세요"
                               required>
                        <p class="text-xs text-gray-500 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            유튜브, 비메오 등 임베드 가능한 영상 주소를 입력해주세요.
                        </p>
                        @error('mq_video_url')
                            <p class="text-sm text-red-500 flex items-center mt-1">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    ...
                                    @foreach($post->mq_image as $index => $filename)
                                        <div class="relative group" id="existing-image-{{ $index }}">
                                            <div class="aspect-square rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                                                <img src="{{ asset('storage/uploads/board_video/' . $filename) }}" 
                                                     alt="첨부 이미지" 
                    ...
                    <div class="flex items-center justify-between pt-8 border-t border-gray-100 mt-8">
                        <button type="button" 
                                onclick="if(confirm('수정 중인 내용이 저장되지 않고 사라집니다.\n이전 페이지로 돌아가시겠습니까?')) { location.href='{{ route('board-video.show', $post->idx) }}'; }"
...
<script>
    class UploadAdapter {
        ...
        _initRequest() {
            ...
            xhr.open('POST', '{{ route('board-video.upload.image') }}', true);
    ...
    // 이미지 삭제 요청
    async function confirmDeleteImage(btn, filename) {
        ...
        try {
            const response = await fetch(`/board-video/delete-image/${idx}/${filename}`, {
                method: 'POST',
                headers: {
