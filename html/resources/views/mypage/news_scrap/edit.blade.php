...
<div class="min-h-screen bg-primary">
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-8">
            <h1 class="text-2xl font-bold text-gray-900 mb-8">뉴스 스크랩 수정</h1>

            <form action="{{ route('mypage.news-scrap.update', $scrap->idx) }}" method="POST" class="space-y-6" id="scrapForm">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    @error('mq_url')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="text-sm text-gray-500">URL이 변경되면 썸네일 이미지가 자동으로 다시 가져와집니다.</p>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-6">
                    <a href="{{ route('mypage.news-scrap.show', $scrap->idx) }}"
