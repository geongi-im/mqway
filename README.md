...
계산한 연령대입니다. "선택한 이유"는 [AI분석] 의 필수 입력이라 비어 있으면 화면과
서버 양쪽에서 분석 요청을 막습니다. 이 값은 `[파트 4]` 질문에만 쓰고, 해석 / 용어 / 전망은
기사만 근거로 씁니다. 프로필 항목이 늘어나면 `BoardScrapController::buildUserContext()`에
